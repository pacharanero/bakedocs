...
Each approved `v<version>` Git tag may publish a deterministic `bakedocs-<version>.tar.gz`, the public `install.sh`, and `SHA256SUMS` through the manually dispatched GitHub `release` environment. Release automation refuses a tag that does not exactly match `bakedocs version`. The Linux/macOS bootstrap discovers the latest release only when no version is selected, permits HTTPS downloads only, bounds downloads and extraction, verifies the archive checksum before extraction, accepts only the exact regular-file payload beneath its single versioned root, confirms the extracted command version and GPL licence text, and then invokes the guarded checkout installer or uninstaller. It does not download mutable branch archives, edit shell startup files, invoke `sudo`, or run a package manager. It fails before downloading when Bash is older than version 4 and prints operating-system-specific routes for obtaining Bash 4, Pandoc 3.7.1 or later, Chromium, and Poppler; native Windows users are directed to WSL. Release checksums detect corruption and asset mismatch but do not provide an authenticity root independent of HTTPS, GitHub account security, and release-environment approval.
