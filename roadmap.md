...
- [ ] **R13 - Add a release-backed bootstrap installer**: publish versioned runtime payloads and `SHA256SUMS`, then provide a copy-and-paste installer that verifies the selected release, installs `bakedocs`, and explicitly offers supported Pandoc, Chromium, and Poppler installation paths for detected operating systems. Do not download mutable branch archives, run an unverified remote payload, silently invoke `sudo`, or assume distribution packages satisfy the Pandoc 3.7.1 minimum.
